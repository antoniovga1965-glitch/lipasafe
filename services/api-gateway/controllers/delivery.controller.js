...
const beforePhoto = async (req, res) => {
  try {
    ...
    if (!req.files?.length) return res.status(400).json({ success: false, message: 'No photo uploaded.' })
    ...
    const result = await uploadBeforePhoto({
      orderId,
      deliveryGuyPhone,
      photos: req.files.map(f => ({ cloudinaryUrl: f.path, cloudinaryPublicId: f.filename })),
    })
