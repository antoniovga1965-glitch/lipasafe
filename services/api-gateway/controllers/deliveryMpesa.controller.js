...
const deliveryMpesaCallback = async (req, res) => {
  ...
  try {
    ...
    const { createAndSend: _pushDel } = require('../src/services/notificationService')
    // Notify buyer — payment held
    await _pushDel({ userId: order.buyerId, type: 'NEW_DELIVERY_ORDER', deliveryOrderId: order.id,
      messageEn: `Payment held. Waiting for delivery agent to upload before-photo.` }).catch(() => {})
    // Notify delivery guy if registered
    const _dgv = order.deliveryGuyPhone.startsWith('254') ? ['0'+order.deliveryGuyPhone.slice(3), order.deliveryGuyPhone] : [order.deliveryGuyPhone, '254'+order.deliveryGuyPhone.slice(1)]
    const _dgu = await prisma.user.findFirst({ where: { phone: { in: _dgv } }, select: { id: true } })
    if (_dgu) await _pushDel({ userId: _dgu.id, type: 'NEW_DELIVERY_ORDER', deliveryOrderId: order.id,
      messageEn: `New delivery job! Goods: ${order.goods}. KES ${order.amount} held in escrow. Open app to upload before-photo.` }).catch(() => {})
