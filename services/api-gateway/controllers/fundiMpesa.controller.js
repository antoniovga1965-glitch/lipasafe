...
const fundiMpesaCallback = async (req, res) => {
  ...
  try {
    ...
    const { createAndSend: _pushFM } = require('../src/services/notificationService')
    await _pushFM({ userId: job.buyerId, type: 'FUNDI_PAYMENT_HELD', fundiJobId: job.id,
      messageEn: `KES ${job.amount} held in escrow. Fundi has been notified to accept the job.` }).catch(() => {})
