...
const { calcFeesSecondHand } = require('../src/utils/feeCalculator')
const { releaseToSeller, refundBuyer } = require('../src/services/secondHandService')
...
const buyListingSchema = z.object({
  inspectionHours: z.number()
    .int()
    .min(MIN_INSPECTION_HOURS, `inspectionHours must be at least ${MIN_INSPECTION_HOURS}`)
    .max(MAX_INSPECTION_HOURS, `inspectionHours must be at most ${MAX_INSPECTION_HOURS}`)
    .default(24),
...
const disputeSchema = z.object({
  reason:      z.enum(['not_as_described', 'damaged_goods', 'wrong_item', 'fake_or_clone', 'not_delivered', 'fraud_suspected', 'other']),
...
const buyListing = async (req, res) => {
  try {
    ...
    try {
      ...
      const { createAndSend: _n1 } = require('../src/services/notificationService')
...
const verifyHandoverOtp = async (req, res) => {
  try {
    ...
    const { createAndSend: _n3 } = require('../src/services/notificationService')
...
const acceptItem = async (req, res) => {
  try {
    ...
    const { createAndSend: _n4 } = require('../src/services/notificationService')
...
const openSecondHandDispute = async (req, res) => {
  try {
    const { id }  = req.params
    const buyerId = req.user.userId
    console.log('DISPUTE_BODY:', JSON.stringify(req.body));
    console.log('DISPUTE_FILES:', req.files?.length);
  ...
  where: {
    ...
    const { createAndSend: _n } = require('../src/services/notificationService')
...
const sellerRespondToDispute = async (req, res) => {
  try {
    ...
    if (parsed.data.accepts) {
      ...
      const { createAndSend: _notify } = require('../src/services/notificationService')
...
const dealBuy = async (req, res) => {
  ...
  return safeError(
    ...
    try {
      ...
      const { createAndSend: _n2 } = require('../src/services/notificationService')
