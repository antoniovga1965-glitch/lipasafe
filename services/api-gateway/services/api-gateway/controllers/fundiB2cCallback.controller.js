    ...
    if (ResultCode === 0) {
      if (type === 'payout') {
        ...
        if (refundUpdated.count === 0) {
          ...
          const { createAndSend: _pushRef } = require('../src/services/notificationService')
          await _pushRef({ userId: job.buyerId, type: 'FUNDI_REFUND_SENT', fundiJobId: jobId,
            messageEn: `Refund of KES ${job.amount} has been sent to your M-Pesa. Ref: ${mpesaRef}` }).catch(() => {})
