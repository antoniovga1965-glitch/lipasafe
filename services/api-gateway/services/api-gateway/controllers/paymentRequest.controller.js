...
const payRequest = async (req, res) => {
  try {
    ...
    const { createAndSend: _pushPR } = require('../src/services/notificationService')
    // Notify requester — they got paid
    await _pushPR({ userId: request.requesterId, type: 'money_request_received', requestId: request.id,
      messageEn: `${sender.name || sender.phone} paid your request of KES ${amount}. Ref: ${txReference}` }).catch(() => {})
    // Notify sender — payment went through
    await _pushPR({ userId: senderId, type: 'payment_sent', requestId: request.id,
      messageEn: `Payment of KES ${amount} sent successfully. Ref: ${txReference}` }).catch(() => {})
