...
const { calcFeesRequestMoney } = require('../src/utils/feeCalculator')
...
const createRequest = async (req, res) => {
  ...
  try {
    ...
    // ── Fees ──────────────────────────────────────────────────────────────────
    const fees = calcFeesRequestMoney(amount)
    ...
    const request = await prisma.requestMoney.create({
      data: {
        ...
        amount:          +fees.amount.toFixed(2),
        platformFee:     +fees.platformFee.toFixed(2),
        b2cCost:         +fees.b2cCost.toFixed(2),
        recipientPays:   +fees.recipientPays.toFixed(2),
        purpose,
        note:            note?.trim() || null,
        state:           'PENDING',
        expiresAt,
