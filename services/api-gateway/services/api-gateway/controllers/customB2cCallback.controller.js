  ...
  try {
    ...
    if (Number(ResultCode) === 0) {
      if (type === 'payout') {
        ...
        const { createAndSend: _pushCP } = require('../src/services/notificationService')
        await _pushCP({ userId: escrow.buyerId, type: 'custom_escrow_completed', customEscrowId: escrowId,
          messageEn: `Deal "${escrow.title}" complete. Payment released. Ref: ${mpesaRef}` }).catch(() => {})
        const _cpv = escrow.counterpartyPhone.startsWith('254') ? ['0'+escrow.counterpartyPhone.slice(3), escrow.counterpartyPhone] : [escrow.counterpartyPhone, '254'+escrow.counterpartyPhone.slice(1)]
        const _cpu = await prisma.user.findFirst({ where: { phone: { in: _cpv } }, select: { id: true } })
        if (_cpu) await _pushCP({ userId: _cpu.id, type: 'custom_escrow_completed', customEscrowId: escrowId,
          messageEn: `KES ${Number(escrow.counterpartyReceives).toFixed(2)} sent to your M-Pesa for deal "${escrow.title}". Ref: ${mpesaRef}` }).catch(() => {})
        ...
        const { createAndSend: _pushCR } = require('../src/services/notificationService')
        await _pushCR({ userId: escrow.buyerId, type: 'custom_escrow_refunded', customEscrowId: escrowId,
          messageEn: `Refund of KES ${Number(escrow.amount).toFixed(2)} sent to your M-Pesa for deal "${escrow.title}". Ref: ${mpesaRef}` }).catch(() => {})
