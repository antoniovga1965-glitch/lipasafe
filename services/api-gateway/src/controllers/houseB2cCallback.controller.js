        ...
        await prisma.$transaction([
          ...
          ...(houseFee > 0 ? [
            prisma.wallet.update({
              where: { id: process.env.PLATFORM_WALLET_ID },
              data: {
                availableBalance: { increment: houseFee },
                totalIn:          { increment: houseFee },
                lastUpdated:      new Date()
              }
            }),
            prisma.walletTransaction.create({
              data: {
                toWalletId: process.env.PLATFORM_WALLET_ID,
                type:       'platform_fee',
                amount:     houseFee,
                reference:  escrowId,
                note:       'Platform fee from house escrow payout',
                status:     'completed'
              }
            })
          ] : [])
