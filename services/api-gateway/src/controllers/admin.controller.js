...
const { releaseToSeller, refundBuyer: secondHandRefund, partialRefund } = require('../services/secondHandService')
const { releaseFunds: bundleRelease, refundBuyer: bundleRefund } = require('../services/bundleService')
const logger = require('../middleware/layer1-gate/httpLogger')
...
async function getDashboardStats(req, res) {
  try {
    ...
    const [
      ...
      walletHeld, protectedTransferPending,
      ...
      prisma.protectedTransfer.count({ where: { state: 'PENDING' } }),
    ...
    const heldEscrowCount = heldTxs._count + fundiHeld._count + deliveryHeld._count + houseHeld._count + customHeld._count + orderHeld._count + walletHeld._count
    const heldEscrowAmount = Number(heldTxs._sum.amount || 0) + Number(fundiHeld._sum.amount || 0) + Number(deliveryHeld._sum.amount || 0) + Number(houseHeld._sum.amount || 0) + Number(customHeld._sum.amount || 0) + Number(orderHeld._sum.amount || 0) + Number(walletHeld._sum.amount || 0)
  ...
  try {
    ...
    const normalize = () => [
      ...
      ...fundiDisputes.map(d => ({
        ...
        opener: d.opener, buyerEvidence: { photos: d.evidencePhotos }, sellerEvidence: null,
...
const { resolveDispute: resolveDeliveryDisputeSvc } = require('../services/disputeService')
  ...
  try {
    ...
    const [disputes, total] = await Promise.all([
      prisma.fundiDispute.findMany({
        ...
        orderBy: { createdAt: 'desc' },
        include: {
          job:    { select: { id: true, description: true, fundiPhone: true, amount: true } },
          opener: { select: { fullName: true, phone: true } },
        },
      }),
...
const getTransactions = async (req, res) => {
  try {
    ...
    const [txs, fundiJobs, houseEscrows, walletTxs, protectedTransfers] = await Promise.all([
      prisma.transaction.findMany({
        orderBy: { createdAt: 'desc' },
        include: {
    ...
    const merged = [...txs, ...normalizedFundi, ...normalizedHouse, ...normalizedWallet, ...normalizedProtectedTransfer].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
