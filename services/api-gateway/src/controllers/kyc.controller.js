...
const { checkTrustedEligibility, promoteTrusted, getVerifiedSellers } = require('../services/kycService')
...
const submitDocs = async (req, res) => {
  try {
    ...
    const CATEGORY_MAP = {
      ...
      'Delivery':    'delivery',
      'House':       'house_agent',
      'Custom':      'other', // no direct enum match — review if Custom needs its own value
    }
    ...
    if (!payment) {
      return res.status(403).json({ success: false, message: 'Pay KES 10 verification fee first' })
    }
