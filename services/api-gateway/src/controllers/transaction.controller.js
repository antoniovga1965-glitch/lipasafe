...
const { calcFees, b2cCost, b2bCost } = require('../utils/feeCalculator');
...
const initiate = async (req, res) => {
  try {
    ...
    // absorb ceiling rounding into platform fee — matches frontend calcFeesGeneric exactly
    ...
    if (!seller.wallet?.isGhost) {
      const { createAndSend: _bn1 } = require('../services/notificationService')
...
const sellerReject = async (req, res) => {
  try {
    ...
    const { createAndSend: _bn2 } = require('../services/notificationService')
...
const verifyOtp = async (req, res) => {
  try {
    ...
    const { createAndSend: _bn3 } = require('../services/notificationService')
