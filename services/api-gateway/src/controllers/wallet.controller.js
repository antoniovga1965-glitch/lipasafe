...
let _platformWalletId = null;
const getPlatformWalletId = async () => {
  if (!PLATFORM_WALLET_ID) throw new Error("PLATFORM_WALLET_MISSING");
  if (_platformWalletId) return _platformWalletId;
  const wallet = await prisma.wallet.findFirst({
    where: { user: { email: "platform@lipasafe.co.ke" } },
    select: { id: true },
  });
  if (!wallet) throw new Error("PLATFORM_WALLET_MISSING");
  _platformWalletId = wallet.id;
  return _platformWalletId;
};
...
const sendMoney = async (req, res) => {
  // Hoisted above try — catch is a SIBLING scope, not a child, so anything
  // declared with const/let inside try is invisible to catch. Plain var
  // bindings up here, plain assignment (no const/let) down below.
