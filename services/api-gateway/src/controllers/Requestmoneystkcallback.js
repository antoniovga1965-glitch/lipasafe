...
const handleStkCallback = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Log as critical for manual resolution. Do NOT revert state.
      logger.error('CRITICAL: B2C payout failed after successful STK', {
        requestId:    request.id,
        requesterPhone,
        amount:       request.amount,
        err:          b2cErr.message,
    ...
    try {
      await prisma.wallet.updateMany({
        where: { user: { phone: process.env.PLATFORM_WALLET_PHONE } },
        data:  {
          availableBalance: { increment: Number(request.platformFee) },
          totalIn:          { increment: Number(request.platformFee) },
