...
import { View, Text, StyleSheet, TouchableOpacity, Image, ScrollView, Alert, ActivityIndicator, Modal, Pressable } from 'react-native';
...
export default function SecondHandMarketScreen({ navigation }) {
  ...
  const [inspectionWindow, setInspectionWindow] = useState(24);
  ...
  const next = () => {
    ...
    if (method === 'pochi' && !isValidKenyanPhone(sellerPhone)) {
      Alert.alert('Invalid Phone', 'Enter a valid Kenyan phone number for the seller.');
      return;
    }
    ...
    // One idempotency key per deal — backend uses this to deduplicate STK pushes
    const clientRef = genClientRef();

    const fee   = calcFee(parsedAmount).toFixed(2);
    const total = (parsedAmount + parseFloat(fee)).toFixed(2);
    const target = method === 'pochi' ? sellerPhone.trim() : `Till ${sellerTill.trim()}`;
...
Inspection: ${inspectionWindow}h`,
      [
        { text: 'Cancel', style: 'cancel', onPress: () => { submitting.current = false; } },
        {
          text: 'Pay Now',
          onPress: async () => {
            let photoUrls = [];
            if (photos.length > 0) {
              try {
                ...
                photos.forEach((uri, i) => {
                  ...
                  form.append('photos', { uri, name: `photo_${i}.${ext}`, type: `image/${ext}` });
                });
                const res = await fetch(`${BASE_URL}/second-hand/upload-photos`, {
                  method: 'POST',
                  headers: { Authorization: `Bearer ${token}` },
                  body: form,
                });
                const data = await res.json();
                if (!res.ok || !data.urls) throw new Error(data.message || 'Upload failed');
                photoUrls = data.urls;
              } catch (err) {
                setLoadingMsg(null);
                Alert.alert('Photo Upload Failed', err.message || 'Could not upload photos. Proceeding without them.');
              }
            }
            ...
            setTimeout(() => { submitting.current = false; }, 3000);
          }
        }
      ]
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <LipaInput
          label={method === 'pochi' ? "Seller Phone Number *" : "Seller Phone (for SMS notification) *"}
          value={sellerPhone}
        ...
        <Text style={styles.hint}>This locks the dispute baseline. Be honest.</Text>
        <View style={styles.condWrap}>
          {CONDITION_OPTIONS.map((c) => (
            ...
            >
              <Text style={[styles.condLabel, condition === c.value && styles.condLabelActive]}>{c.label}</Text>
              <Text style={[styles.condDesc,  condition === c.value && styles.condDescActive]}>{c.desc}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <Text style={styles.hint}>How long buyer has to raise a dispute after receiving the item.</Text>
        <View style={styles.row}>
          {INSPECTION_WINDOWS.map((w) => (
            <TouchableOpacity
              key={w.value}
              style={[styles.chip, inspectionWindow === w.value && styles.chipActive]}
              onPress={() => setInspectionWindow(w.value)}
            >
              <Text style={[styles.chipText, inspectionWindow === w.value && styles.chipTextActive]}>{w.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <Text style={styles.hint}>Capture item state now. Used as dispute evidence baseline.</Text>
...
const styles = StyleSheet.create({
  ...
  chip:           { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20, borderWidth: 1, borderColor: colors.grayDark + '40', backgroundColor: colors.gray },
  chipActive:     { backgroundColor: colors.primary, borderColor: colors.primary },
  chipText:       { fontSize: 13, fontWeight: '600', color: colors.black },
  chipTextActive: { color: colors.white },
  condWrap:       { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 },
  condChip:       { width: '47%', padding: 12, borderRadius: 14, borderWidth: 1, borderColor: colors.grayDark + '30', backgroundColor: colors.gray },
  ...
  thumbWrap:      { position: 'relative', width: 90, height: 90, borderRadius: 12, overflow: 'hidden' },
  thumb:          { width: 90, height: 90, borderRadius: 12, backgroundColor: colors.gray },
  ...
  photoBtn:       { width: 90, height: 41, borderRadius: 12, borderWidth: 1, borderColor: colors.primary + '40', backgroundColor: colors.primary + '10', alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 4 },
  photoBtnText:   { fontSize: 11, color: colors.primary, fontWeight: '600' },
  overlay:        { flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', alignItems: 'center', justifyContent: 'center' },
  overlayCard:    { backgroundColor: '#fff', borderRadius: 20, padding: 36, alignItems: 'center', gap: 16, minWidth: 200 },
